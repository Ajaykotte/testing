...
try{
	...
	Connection con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","Rohith","task24");
	Statement st = con.createStatement();

	ResultSet rs=st.executeQuery("SELECT max(id) FROM StudentDetails");

	if(rs.next()){
	id=rs.getInt(1);
	id++;

	PreparedStatement p=con.prepareStatement("insert into  StudentDetails values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
